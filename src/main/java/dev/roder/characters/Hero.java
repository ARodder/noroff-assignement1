...
public abstract class Hero {
     ...
     */
    public Hero(String name, int strength, int dexterity, int intelligence, ArrayList<WeaponType> validWeaponTypes,
            ArrayList<ArmorType> validArmorTypes) {
        this.name = name;
        this.level = 1;
        this.maxHealth = 10;
    ...
    /**
     * Levels up character, abstract since different classes does not level up the
     * same way.
     */
    public abstract void levelUp();

    /**
     * Equips a new weapon if the weapon is a valid weapon for the hero type, nad
     * the hero is at or above the required level.
     * 
     * @param newWeapon new weapon to equip on the hero.
     */
    public void equip(Weapon newWeapon) throws InvalidWeaponException {
        if (!this.validWeaponTypes.contains(newWeapon.getWeaponType()))
            throw new InvalidWeaponException("The type of weapon can not be used on this type of hero");
        if (!(this.level >= newWeapon.getRequiredLevel()))
            throw new InvalidWeaponException("Your hero is not high enough level for this weapon yet");
        equipment.put(SlotType.Weapon, newWeapon);

    }

    /**
     * Equips a new armor-piece if the piece is valid for the hero type, and the
     * hero is at or above the required level.
    ...
    public void equip(Armor newArmor) throws InvalidArmorException {
        if (!this.validArmorTypes.contains(newArmor.getArmorType()))
            throw new InvalidArmorException("The type of armor can not be used on this type of hero");
        if (!(this.level >= newArmor.getRequiredLevel()))
            throw new InvalidArmorException("Your hero is not high enough level for this armor yet");
    ...
    public HeroAttribute totalAttributes() {
        HeroAttribute[] gear = new HeroAttribute[] {
                equipment.get(SlotType.Head) != null ? ((Armor) equipment.get(SlotType.Head)).getArmorAttribute()
                        : null,
                equipment.get(SlotType.Body) != null ? ((Armor) equipment.get(SlotType.Body)).getArmorAttribute()
                        : null,
    ...
    public String display() {
        ...
        builder.append("     Weapon: "
                + (equipment.get(SlotType.Weapon) == null ? "none" : equipment.get(SlotType.Weapon).getName()) + "\n");
        builder.append("     Head: "
                + (equipment.get(SlotType.Head) == null ? "none" : equipment.get(SlotType.Head).getName()) + "\n");
        builder.append("     Body: "
                + (equipment.get(SlotType.Body) == null ? "none" : equipment.get(SlotType.Body).getName()) + "\n");
        builder.append("     Legs: "
                + (equipment.get(SlotType.Legs) == null ? "none" : equipment.get(SlotType.Legs).getName()) + "\n");
